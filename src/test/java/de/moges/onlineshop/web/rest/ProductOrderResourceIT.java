...
class ProductOrderResourceIT {
    ...
    public static ProductOrder createUpdatedEntity(EntityManager em) {
        ProductOrder updatedProductOrder = new ProductOrder().placedDate(UPDATED_PLACED_DATE).status(UPDATED_STATUS).code(UPDATED_CODE);
        // Add required entity
        Customer customer;
        if (TestUtil.findAll(em, Customer.class).isEmpty()) {
    ...
    void partialUpdateProductOrderWithPatch() throws Exception {
        ...
        partialUpdatedProductOrder.status(UPDATED_STATUS).code(UPDATED_CODE);
